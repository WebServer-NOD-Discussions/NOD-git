...
def searchInTbl(qry,trgt,file,keyword):
	...
	if match:
		...
		return(qry, trgt, keyword,"Sorry!","The id could","NOT BE FOUND")
...
def DBmap(mode,table,target):
	...
	for row in F_tbl.index:
		...
		chkLog.write("\n\tExtracted DBids from jackHmmer:\t%s\n" %comment)
...
#Reading the input mode by the user and defining workflow accordingly.
	if sum(1 for line in open(resDir+"/FilteredTbl.hmmer")) <= 1 : #to check if any filtered hits were found or an empty table is generated.
		htmlErrors.write("SORRY!! None of the target homologs identified for your query protein(s) could pass the alignment coverage filter. Please find the link to the table containing alignment coverage (The current coverage threshold is 70%).\n\nHere is a link to the unfiltered table before using the coverage threshold.\n\t http://pauling..mbu.iisc.ac.in/nod_de/%s/UnFiltered.tbl" %actDir); exit(0);
